...
@section('content')
	...
	<div class="app-page-title">
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>ITEM SERVER UPDATE FOR {{$server->server_number}}
	...
	<div class="row">
		...
		<div class="col-md-12">
			...
			<a href="{{route('serversview.show',['name'=>$server->servername->name])}}">
					  ...
					  <div class="form-group">
					  	<div class="table-responsive">
							<table class="mb-0 table" style="color:white">
								...
								<tr>
								  <td colspan="6" style="border-top: 1px solid #304155; color:white"><h4 align="center">View Selected Stored Items For Server</h4></td>
							    </tr>
							    <tr>
							    	<td style="color:white;text-align: center;">FTTH</td>
							    	<td style="color:white;text-align: center;">ITEM NAME</td>
								...
								<tbody>
									@foreach(session('itemsserversdb') as $k=>$val)
									 @if(is_array($val))
									  @if($store != null)
											...
											<div class="modal fade deleteitem{{$itemval->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
														...
														<form method="get" action="{{route('removeolditemssamdm',['itemname_id'=>$val['id'],'id'=>$itemval->id,'server_id'=>$server->id])}}" >
															<div class="modal-body">
											...
											<div class="modal fade deleteitemcount{{$itemc->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
														...
														<form method="get" action="{{route('removeolditemssamdm',['itemname_id'=>$val['id'],'id'=>$itemc->id,'server_id'=>$server->id])}}" >
															<div class="modal-body">
			...
			<div class="col-md-12">
				<div class="main-card mb-3 card">
					<div class="card-body">
						...
						<!---------------------------->
						<form method="post" action="{{route('server.update',['server_number'=>$server->server_number])}}" enctype="multipart/form-data" id="formnameupload">
                           <input type="hidden" name="_token" value="{{csrf_token()}}">
								 ...
								 <div class="col-md-6 pr-md-1">
								    <div class="form-group">
								        ...
								        <select type="select"name="servername_id" id="servername_id" class="form-control js-example-tags custom-select" >
										 <option value="{{$server->servername_id}}">{{$server->servername->name}}</option>
										 @foreach($servernames as  $servername)
										 @if($server->servername_id != $servername->id)
								 ...
								 <div class="col-md-6 pr-md-1">
								    <div class="form-group">
								        ...
								        <select type="select"name="staff_id" id="staff_id" class="form-control js-example-tags1 custom-select" >
										 <option value="{{$server->staff_id}}">{{$server->staff->name}}</option>
										 @foreach($staffs as  $staff)
										  @if($server->staff_id != $staff->id)
								    ...
								    <div class="form-group">
								        ...
								        @if($server->staff_id)
								         <select name="positions" class="form-control" id="positions">
								         	<option>{{$server->staff->jobtitle['name']}}</option>
								        </select>
								        @else
								       <select name="positions" class="form-control" id="positions">
								        </select>
								 ...
								 <div class="col-md-6 pr-md-1">
								    ...
								    <div class="form-group">
								    	<label>  </label>
								         <br/>
								    	@if($server->sign_file)
					                           <button type="button" class="btn" data-toggle="modal" data-target=".myimage{{$server->id}}"> <img class="image" src="{{asset('images/ftth/servers/'.$server->sign_file)}}" alt="" style="border:1px solid#777; background: #fff"></button>
										...
										<div class="modal fade myimage{{$server->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
											<div class="modal-dialog modal-lg">
												<div class="modal-content">
													<div class="modal-header">														
													...
													<div class="modal-body">
														 <img src="{{asset('images/ftth/servers/'.$server->sign_file)}}" class="image2" >
													</div>												
