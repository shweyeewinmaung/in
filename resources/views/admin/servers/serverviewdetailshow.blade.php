	...
	<div class="app-page-title">
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>{{$name}} Sever For {{$server_number}}
	...
	<div class="row">
		...
		<div class="col-md-12">
			@if(\Auth::user()->isSuper() || \Auth::user()->hasPermission("view-itemserver"))
			   <a href="{{route('serversview.show',['name'=>$name])}}">
								...
								<thead style="background: #fff;">
									 <tr>
									 	 <th style="border-bottom: none" rowspan="4">
									 	 	@if($serverdetail->sign_file)
					                           <button type="button" class="btn" data-toggle="modal" data-target=".myimage{{$serverdetail->id}}"> <img class="image" src="{{asset('images/ftth/servers/'.$serverdetail->sign_file)}}" alt="" style="border:1px solid#777; background: #fff"></button>
										...
										<div class="modal fade myimage{{$serverdetail->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
											<div class="modal-dialog modal-lg">
												<div class="modal-content">
													...
													<div class="modal-body">
														 <img src="{{asset('images/ftth/servers/'.$serverdetail->sign_file)}}" class="image2" >
													</div>												
										...
										<!-- ------------------Picture modal End------------ -->
					                     ...
					                     <th colspan="2" style="border-top: none;border-bottom: none; background: #5a6471;color: #fff;">
					                      	SERVER CODE - {{$serverdetail->server_number}}
					                      </th> 
					                      </tr>
					                      <tr>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">STORE - {{$serverdetail->store->name}}</label></th>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">STAFF - {{$serverdetail->staff->name}}</label></th>
					                     </tr>
					                     <tr>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">STATUS - {{$serverdetail->status}}</label></th>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">BY -  {{$serverdetail->user->name}}</label></th>
					                     </tr>
					                      <tr>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;">@if($serverdetail->status =='confirm')<label style="color: #fff;">CONFIRM BY - {{$serverdetail->confirmuser->name}}</label>@endif</th> 
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">CREATED - {{ date('Y-m-d', strtotime($serverdetail->created_at)) }} ({{$serverdetail->created_at->format('H:i:s')}})</label><br><label style="color: #fff;">
					                      UPDATED - {{ date('Y-m-d', strtotime($serverdetail->updated_at)) }} ({{$serverdetail->updated_at->format('H:i:s')}})</label></th>              
									 </tr>
								</thead>
								...
								<table class="table table-bordered" style="background: #2b3c51;color: #fff;">
									...
									<thead style="background: #2b3c51;color: #fff;">
										<tr>
						                     ...
						                     <th>MODEL</th>
						                     <th>DAMAGE</th>
						                     <th>@if(\Auth::user()->isSuper() || \Auth::user()->hasPermission("edit-itemserver") ||  \Auth::user()->hasPermission("delete-itemserver"))
									   ACTION
									 @endif</th>						                     
						                </tr>									 
									</thead>
											...
											<div class="modal fade edititem{{$itemsm->item_id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
									                  ...
									                  <form method="post" action="{{route('ItemByItemServerUpdate',['id'=>$itemsm->item_id])}}" enctype="multipart/form-data" >
															<div class="modal-body">
											...
											<div class="modal fade edititemwithcount{{$itemsarray['id']}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
									                  ...
									                  <form method="post" action="{{route('ItemByItemServerCountUpdate',['itemname_id'=>$itemsarray['id'],'server_id'=>$serverdetail->id])}}" enctype="multipart/form-data" >
															<div class="modal-body">
											...
											<div class="modal fade editredoitemwithcount{{$itemsarray['id']}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
									                  ...
									                  <form method="post" action="{{route('ItemByItemServerRedoCountUpdate',['itemname_id'=>$itemsarray['id'],'server_id'=>$serverdetail->id])}}" enctype="multipart/form-data" >
															<div class="modal-body">
