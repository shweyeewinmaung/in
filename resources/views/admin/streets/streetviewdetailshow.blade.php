	...
	<div class="app-page-title">
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>{{$name}} STREET FOR {{$street_number}}
	...
	<div class="row">
		...
		<div class="col-md-12">
			@if(\Auth::user()->isSuper() || \Auth::user()->hasPermission("view-itemstreet"))
			   <a href="{{route('streetview.show',['name'=>$name])}}">
								...
								<thead style="background: #fff;">
									 <tr>
									 	 <th style="border-bottom: none" rowspan="4">
									 	 	@if($streetdetail->street_sign_file)
					                           <button type="button" class="btn" data-toggle="modal" data-target=".myimage{{$streetdetail->id}}"> <img class="image" src="{{asset('images/ftth/streets/'.$streetdetail->street_sign_file)}}" alt="" style="border:1px solid#777; background: #fff"></button>
										...
										<div class="modal fade myimage{{$streetdetail->id}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
											<div class="modal-dialog modal-lg">
												<div class="modal-content">
													...
													<div class="modal-body">
														 <img src="{{asset('images/ftth/streets/'.$streetdetail->street_sign_file)}}" class="image2" >
													</div>												
										...
										<!-- ------------------Picture modal End------------ -->
					                     ...
					                     <th colspan="2" style="border-top: none;border-bottom: none; background: #5a6471;color: #fff;">
					                      	STREET CODE - {{$streetdetail->street_number}}
					                      </th> 
					                      </tr>
					                      <tr>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">STORE - {{$streetdetail->store->name}}</label></th>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">STAFF - {{$streetdetail->staff->name}}</label></th>
					                     </tr>
					                     <tr>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">STATUS - {{$streetdetail->status}}</label></th>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">BY -  {{$streetdetail->user->name}}</label></th>
					                     </tr>
					                      <tr>
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;">@if($streetdetail->status =='confirm')<label style="color: #fff;">CONFIRM BY - {{$streetdetail->confirmuser->name}}</label>@endif</th> 
					                      <th style="border-top: none;border-bottom: none; background: #5a6471;"><label style="color: #fff;">CREATED - {{ date('Y-m-d', strtotime($streetdetail->created_at)) }} ({{$streetdetail->created_at->format('H:i:s')}})</label><br><label style="color: #fff;">
					                      UPDATED - {{ date('Y-m-d', strtotime($streetdetail->updated_at)) }} ({{$streetdetail->updated_at->format('H:i:s')}})</label></th>              
									 </tr>
								</thead>
											...
											<div class="modal fade edititemwithcount{{$itemsarray['id']}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
									                  ...
									                  <form method="post" action="{{route('ItemByItemStreetCountUpdate',['itemname_id'=>$itemsarray['id'],'street_id'=>$streetdetail->id])}}" enctype="multipart/form-data" >
															<div class="modal-body">
											...
											<div class="modal fade editredoitemwithcount{{$itemsarray['id']}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
									                  ...
									                  <form method="post" action="{{route('ItemByItemStreetRedoCountUpdate',['itemname_id'=>$itemsarray['id'],'street_id'=>$streetdetail->id])}}" enctype="multipart/form-data" >
															<div class="modal-body">
