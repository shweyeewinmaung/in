...
@section('content')
	...
	<div class="row">
		...
		<form class="search-bar" style="float: right" action="{{route('getsearchlistcustomernameupdate.search',['storename'=>$storename,'customer_id'=>$customer_id])}}" method="get">
					{{csrf_field()}}
							...
							<table class="mb-0 table">
					            ...
					            @foreach($customernameslists as $key=>$customernamelist)
									<tr>
										...
										<td>  <a href="{{route('getstoreandcustomernameupdate.get',['customer_id'=>$customer_id,'customernamecode'=>$customernamelist->code])}}">{{ $customernamelist->name }}</a></td>
										<td><a href="{{route('getstoreandcustomernameupdate.get',['customer_id'=>$customer_id,'customernamecode'=>$customernamelist->code])}}">{{ $customernamelist->code }}</a></td>
