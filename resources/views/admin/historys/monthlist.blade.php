...
@section('content')
	...
	<div class="row">
		...
		@if($historymonthlistsall->count() <= 0) 
		<div class="row">
		...
		<div class="row">
			...
			<div class="col-md-12">
				<div class="main-card mb-3 card">
					<div class="card-body" style="padding: 0px">
						<div class="table-responsive">
							<table class="mb-0 table">
								...
								<tbody>
									@foreach($historymonthlistsall as $k=>$history)
									<tr>										 
										<td><a href="{{route('history.daylist',['year'=>$year,'month'=>$k])}}">{{$k}} {{$year}}</a></td>
										<td>
											@if(\Auth::user()->isSuper() ||   \Auth::user()->hasPermission("view-history"))
											<a href="{{route('history.daylist',['year'=>$year,'month'=>$k])}}">
											...
											@if(\Auth::user()->isSuper() ||   \Auth::user()->hasPermission("delete-history"))
										     <button class="mr-2 btn-icon btn-icon-only btn btn-outline-danger" data-toggle="modal" data-target=".deletemonth{{$k}}" title="Delete"><i class="pe-7s-trash btn-icon-wrapper"> </i></button>

											<!-- ------------------Delete Year modal Start------------ -->

											<div class="modal fade deletemonth{{$k}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
														<div class="modal-header">
															<h5 class="modal-title" id="exampleModalLongTitle">TO DELETE FOR {{$k}} in {{$year}}</h5>

															<button type="button" class="close" data-dismiss="modal" aria-label="Close">
																<span aria-hidden="true">&times;</span>
															</button>
														</div>
														<form method="post" action="{{route('history.deletemonth',['year'=>$year,'month'=>$k])}}" >
					...
					<div class="card-body" >
							<p style="float: right;">TOTAL {{$historymonthlistsall->count()}}</p>
