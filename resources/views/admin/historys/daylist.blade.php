...
@section('content')
	...
	<div class="app-page-title">
		<div class="page-title-wrapper">
			<div class="page-title-heading">
				...
				<div>HISTORY LISTS FOR {{$month}} IN {{$year}}
	...
	<div class="row">
		...
		<div class="col-md-12">
			...
			<a href="{{route('history.monthlist',['year'=>$year])}}">
		...
		<div class="row">
			...
			<div class="col-md-12">
				<div class="main-card mb-3 card">
					<div class="card-body" style="padding: 0px">
						<div class="table-responsive">
							<table class="mb-0 table">
								...
								<tbody>
									@foreach($historydaylistsall as $k=>$history)
									<tr>										 
										<td><a href="{{route('history.datelist',['year'=>$year,'month'=>$month,'day'=>$k])}}">{{$k}}-{{$month}}-{{$year}}</a></td>
										<td>
											@if(\Auth::user()->isSuper() ||   \Auth::user()->hasPermission("view-history"))
											<a href="{{route('history.datelist',['year'=>$year,'month'=>$month,'day'=>$k])}}">
											...
											@if(\Auth::user()->isSuper() ||   \Auth::user()->hasPermission("delete-history"))
										     <button class="mr-2 btn-icon btn-icon-only btn btn-outline-danger" data-toggle="modal" data-target=".deleteday{{$k}}" title="Delete"><i class="pe-7s-trash btn-icon-wrapper"> </i></button>

											<!-- ------------------Delete Year modal Start------------ -->

											<div class="modal fade deleteday{{$k}}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
												<div class="modal-dialog modal-lg">
													<div class="modal-content">
														<div class="modal-header">
															<h5 class="modal-title" id="exampleModalLongTitle">TO DELETE FOR {{$k}} in {{$month}} {{$year}}</h5>

															<button type="button" class="close" data-dismiss="modal" aria-label="Close">
																<span aria-hidden="true">&times;</span>
															</button>
														</div>
														<form method="post" action="{{route('history.deleteday',['year'=>$year,'month'=>$month,'day'=>$k])}}" >
					...
					<div class="card-body" >
							<p style="float: right;">TOTAL {{$historydaylistsall->count()}}</p>
